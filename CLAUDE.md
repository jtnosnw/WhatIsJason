...
3. **Firestore holds user data only.** That means profile, prefs, quiz attempts, the review queue and path progress, all under `users/{uid}`, plus the read-only `allowlist`. No content, ever.
